...
interface PaymentMethod {
  id: "ach" | "check" | "realtime" | "prepaid" | "venmo" | "paypal" | "zelle" | "international" | "crypto";
  title: string;
  description: string;
  icon: LucideIcon;
  benefits: string[];
...
export const PaymentMethodCard = ({ method, onSelect }: PaymentMethodCardProps) => {
  ...
  return (
    ...
    >
      {method.fee && (
        <div className="absolute top-3 right-3 bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-sm font-semibold z-10">
          {method.fee}
        </div>
      )}
      {method.ribbon && (
        <div className="absolute bottom-0 right-0 w-40 h-40 overflow-hidden pointer-events-none">
          <div className="absolute bottom-0 right-0 w-48 h-10 bg-blue-500 shadow-lg transform rotate-[-45deg] translate-x-[25%] translate-y-[25%] flex items-center justify-center border-b-2 border-blue-700">
            <span className="text-white font-bold text-sm uppercase tracking-wider whitespace-nowrap pl-12">
              {method.ribbon}
            </span>
          </div>
        </div>
